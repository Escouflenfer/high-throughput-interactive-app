...
W_folderpath = html.Div(children=[
        html.Label('Folderpath'),
        dcc.Dropdown([elm for elm in os.listdir('./data/EDX/') if not elm.startswith('.')],
                     id='folderpath')

    ], className='cell11'
 )
# Folderpath for the EDX map
W_folderpath_edx = html.Div(children=[
        html.Label('Folderpath'),
        dcc.Dropdown([elm for elm in os.listdir('./data/EDX/') if not elm.startswith('.')],
                     id='folderpath_edx')
...
# Slider X component
W_x_slider = html.Div(children=[
        html.Label('X Position'),
        dcc.Slider(min=-40, max=40, step=5,
                   value=0, 
                   id='x_slider')
...
W_xrange_slider = html.Div(children=[
    html.Label('Energy Range'),
    dcc.RangeSlider(min=0, max=20, step=.1,
                    value=[0, 10],
...
W_yrange_slider = html.Div(children=[
    html.Label('Counts'),
    dcc.RangeSlider(min=0, max=50000, step=1000,
                    value=[0, 15000],
...
W_edx_spectra = html.Div([
                dcc.Graph(id='edx_spectra')
                ], className='plot_cell'
)
# EDX heatmap 
W_edx_heatmap = html.Div([
...
app.layout = html.Div([
    dcc.Tabs(id="tabs", value='tab-1', children=[
        dcc.Tab(id='tab-1', label='EDX Spectra', children=[
            html.Div([
            W_folderpath, W_x_slider, W_xrange_slider,
            W_y_slider, W_yrange_slider,
            W_edx_spectra
            
        ], className='grid_layout')

        ]),
        dcc.Tab(id='tab-2', label='Composition map', children=[
            html.Div([
            W_folderpath_edx,
            W_element,
            W_edx_heatmap
...
@callback(Output('edx_spectra', 'figure'),
...
@callback(Output('edx_heatmap', 'figure'),
          Input('folderpath', 'value'),
...
@callback(Output('folderpath', 'value'),
          ...
          Input('element_edx', 'value'),)
def update_on_tab_change(tab, folderpath, folderpath_edx, element_edx_opt, element_edx_val):
    if tab == 'tab-1':
