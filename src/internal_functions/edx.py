...
def generate_spectra(foldername, x_pos, y_pos):
    ...
    fig=go.Figure(data=[go.Scatter(x=[elm[0] for elm in edx_spectra], y=[elm[1] for elm in edx_spectra], marker_color='purple')])
    fig.update_layout(title=f'EDX Spectrum for {foldername} at position ({x_pos}, {y_pos})', height=700, width=1300)
...
def generate_heatmap(folderpath_edx, element_edx):
    ...
    step_x = 5; step_y = 5; start_x = -40; start_y = -40; 
    X_POS = []; Y_POS = []; ELM = []; 
    ...
    fig.update_layout(title=f'EDX Heatmap for element {element_edx}', height=800, width=800)
